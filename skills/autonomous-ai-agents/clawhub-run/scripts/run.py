#!/usr/bin/env python3
"""n8n entry for clawhub:clawhub-run. Never evals SKILL.md; refuses a same-turn install."""
...
def _has_install(tools: list[str]) -> bool:
    for tool in tools:
        item = tool.lower()
        if item.startswith("clawhub:"):
            item = item[len("clawhub:"):]
        if item in {"install", "clawhub-install"} or item.endswith("-install"):
            return True
    return False
...
def main(argv: list[str] | None = None) -> int:
    ...
    body = json.dumps({
        ...
        "tools": tools or ["clawhub:clawhub-run"],
    }).encode("utf-8")
    request = Request(
